---
title: Ebb — Terms of Use
---

# Terms of Use for Ebb

**Last updated: 10/09/2026**

## Agreement

By downloading or using Ebb, you agree to these terms. If you do not
agree, please do not use the app.

Ebb is distributed through Apple's App Store and is also subject to
Apple's Licensed Application End User License Agreement, available at
<https://www.apple.com/legal/internet-services/itunes/dev/stdeula/>.
Where these terms and Apple's standard EULA conflict, Apple's terms
apply.

## What Ebb does

Ebb visualizes the time remaining until dates you choose. It is a
countdown and motivation tool. It is not a calendar, a task manager, or
a reminder service you should rely on for anything time-critical,
medical, legal, or financial.

## Reminders are best-effort

Ebb schedules reminders using iOS local notifications. Their delivery
depends on iOS, on your device's settings, and on system limits outside
Ebb's control — including the operating system's cap on how many
notifications an app may have pending at once. **Do not rely on Ebb for
anything where a missed reminder matters.**

## Ebb Pro

Ebb Pro is a one-time, non-consumable in-app purchase that unlocks
unlimited active goals, iCloud sync, Live Activities, Lock Screen
widgets, and the advanced reminder plans.

It is tied to your Apple Account and can be restored on your devices
using **Restore Purchases** in Settings. Purchases are processed by
Apple; refunds are handled by Apple under their policies, not by us.

Features included in Pro may change over time as the app develops. We
will not remove a feature you have already paid for and move it behind
an additional purchase.

## Your data

Your goals are yours. Ebb stores them on your device and, with Pro and
sync enabled, in your own iCloud account. See the Privacy Policy for
details.

You are responsible for keeping your own backups. Enabling iCloud sync
is not a guarantee against data loss.

## No warranty

Ebb is provided "as is", without warranty of any kind. We do not
warrant that it will be uninterrupted, error-free, or that reminders
will always arrive.

To the fullest extent permitted by law, we are not liable for any
indirect or consequential damages arising from your use of the app,
including missed deadlines or lost data.

## Acceptable use

Do not attempt to reverse-engineer, decompile, or redistribute Ebb
except as permitted by applicable law.

## Changes

These terms may be updated. Continued use after an update constitutes
acceptance of the revised terms.

## Contact

**oleksii.grygoryshyn@gmail.com**
